...
public class Applicant implements Comparable<Applicant>{
    ...
    public int tripPriority() { return this.tripPurpose == 'S' ? 3 : (this.tripPurpose == 'B' ? 2 : (this.tripPurpose == 'V' ? 1 : 0)); }
    ...
    public int compareTo(Applicant a){
        ...
        if(expired && !a.expired) { return 1; }
        if(!expired && a.expired) { return -1; }
        if(tripPriority() > a.tripPriority()) { return 1; }
        if(tripPriority() < a.tripPriority()) { return -1; }
        if(tripPriority() == 0) return 0;
        return Long.compare(a.tripDate.getTime(), tripDate.getTime());
    }
}
